...
export default {
  ...
  methods:{
    ...
    handleNext() {
      var student_id = window.localStorage.getItem('student_id')
      var pici_id =this.$route.query.pici_id
      this.$router.push({ path: '/finish', query: {pici_id:pici_id, openid: this.$route.query.openid}})
    },
      ruxuexuzhi(){
          this.$router.push({ path: '/admissionInfo', query: {openid: this.$route.query.openid}})
      },
