    ...
    <div class="main-page">
        ...
        <div style="height: calc(100% - 50px);overflow: auto">
            <div class="form-content" style="height: auto">
                <div class="weixtishi"><i class="el-icon-warning"/> 学生信息</div>
                <el-form
                    label-position="left"
                    inline size="mini"
                    class="student-info">
                    ...
                    <el-row>
                        <el-col :span="24">
                            <el-form-item class="studentInfo"  label="毕业中学：">
                                <span>{{ studentInfo.grade_school }}</span>
                            </el-form-item>
                        </el-col>
                    ...
                    <el-row>
                    <el-col :span="24">
                        <el-form-item class="studentInfo"  label="联系电话：">
                    ...
                    <el-row>
                        <el-col :span="24">
                            <el-form-item class="studentInfo"  label="身份证号：">
                                <span>{{ studentInfo.idcode }}</span>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    ...
                    <el-row>
                        <el-col :span="24">
                            <el-form-item class="studentInfo"  label="当前班级：">
                    ...
                    <!--<el-row>-->
                        <!--<el-col :span="12">-->
                            <!--<el-form-item class="studentInfo" :label-width="InfoLabelWidth" label="交费：">-->
                                <!--<i v-if="studentInfo.is_pay===0" class="el-icon-close" style="color:firebrick" />-->
                                <!--<i v-if="studentInfo.is_pay===1" class="el-icon-check" style="color:forestgreen" />-->
                            <!--</el-form-item>-->
                        <!--</el-col>-->
                        <!--<el-col :span="12">-->
                            <!--<el-form-item class="studentInfo" :label-width="InfoLabelWidth" label="报到：">-->
                                <!--<i v-if="studentInfo.is_register===0" class="el-icon-close" style="color:firebrick" />-->
                                <!--<i v-if="studentInfo.is_register===1" class="el-icon-check" style="color:forestgreen" />-->
                            <!--</el-form-item>-->
                        <!--</el-col>-->
                    <!--</el-row>-->
                </el-form>
                ...
                <!--<div style="margin-top:0px;padding:10px;">
                    <el-button v-if="studentInfo.is_register == 1" :disabled="Object.keys(studentInfo).length === 0" style="width:100%;margin-top:5px" type="info" @click.native="handleCancel">
                        <i class="fa fa-mail-reply-all " aria-hidden="true"/> 取消报到
                    </el-button>
                    <el-button v-if="studentInfo.is_register == 0"
            ...
            <!--<div style="margin-top:0px;padding:10px;">-->
                ...
                <!--</x-button>-->
            <!--</div>-->
            <div class="form-content" style="margin-top: 20px; height: auto; padding-top: 10px">

                <el-row><span style="font-size: 14px; color: dimgray"><i class="el-icon-sort"></i><strong>选择即将调整到的目标班级</strong></span></el-row>
                <hr style="border: 1px lightgrey solid">
                <el-row style="margin-top: 10px">
                    <span style="font-size: 16px; color: darkred">⋆</span><span style="font-size: 14px; color: dimgray"><strong>目标班级</strong></span>
                    <el-select style="width: 100%" v-model="targetClass">
                        <el-option v-for="item in classLists" :key="item.unitid" :label="item.unitname" :value="item.unitid"></el-option>
                    </el-select>
                </el-row>
                <el-row style="margin-top: 10px">
                    <span style="font-size: 14px; color: dimgray" ><strong>转班原因</strong></span>
                    <el-input v-model="zbyy" type="textarea" :rows="2"></el-input>
                </el-row>

                <el-row style="margin-top: 10px">
                    <x-button type="primary" @click.native="handleClassChange"  plain>确认转班</x-button>
                </el-row>
            </div>
...
<script>
    ...
    import { editStudentInfo, getStudentInfo, getFormConfigLists,setRegister,getStudentChargeItems, getClassLists, modifyStudentClass } from '@/api/student'
    ...
    export default {
        ...
        mounted() {
            ...
            this.fetchStudentInfo(student_id)
            this.fetchClassLists(student_id)
        },
        methods: {
            handleClassChange(){//change student's class
                console.log(this.targetClass)
               if(this.targetClass == this.studentInfo.unit_code){
                    alert("目标班级和原班级相同")
                    console.log("目标班级和原班级相同")
                    return
                }
                this.updateStudentClass(this.$route.query.student_id, this.targetClass, this.zbyy);
            },
            updateStudentClass(student_id, targetClass, zbyy){//
                ...
                modifyStudentClass({student_id: student_id, targetClass: targetClass, zbyy:zbyy}).then(response=>{
            ...
            fetchClassLists(student_id){
                getClassLists({ student_id: student_id }).then(reponse=>{
                    this.classLists = reponse.data
            ...
            fetchStudentInfo(student_id) {
                getStudentInfo({ student_id: student_id }).then(response => {
                    ...
                    this.targetClass = response.data.unit_code
                }).catch(error => {
                    console.log(error)
                })
            },
            ...
            fetchFormFieldLists(student_id) {
                ...
                getFormConfigLists({ form_name: '手机学生完善信息表单', student_id: student_id }).then(response => {
          ...
          uploadImageSuccess(response, file, fileList) {
            ...
            handleSumbit() {
                ...
                _this.$vux.confirm.show({
                    title:'提示',
                    content:'确认【' + this.studentInfo.name + '】已到校报到, 是否继续?',
                    onConfirm () {
                        setRegister({ student_id: _this.studentInfo['id'], is_register: 1 }).then(response => {
            ...
            handleCancel() {
                ...
                _this.$vux.confirm.show({
                    ...
                    onConfirm () {
                        setRegister({ student_id: _this.studentInfo['id'], is_register: 0 }).then(response => {
                            _this.$vux.alert.show({
                                title: '提示',
                                content:'取消成功',
                                onHide () {
