<template>
  <div class="app-container" style="height: 100%;">
    <el-row style="height: 100%;">
      ...
      <el-col style="height:100%;width:calc(100% - 220px); " >
        <el-row STYLE="margin: 3px">
          ...
          <el-button size="mini" type="success" @click.native="handleChangeUnit"><i class="el-icon-sort" aria-hidden="true"/> 批量调整班级</el-button>
          ...
          <el-col :span="24" >
            <el-form :inline="true" :model="params" size="mini">
              ...
              <el-form-item label="关键词:" style="margin-bottom: 0px;margin-top:10px;" >
                <el-input v-model="params.conditions.keyword" style="width:180px" placeholder="姓名/学号/身份证号码"/>
              </el-form-item>
              <el-form-item label="是否交费：" label-width="90px" style="margin-bottom: 0px;margin-top:10px;">
                <el-select v-model="params.is_pay" placeholder="请选择是否交费" style="width:180px">
                  <el-option v-for="item in YesOrNotLists" :key="item.value" :label="item.text" :value="item.value"/>
                </el-select>
              </el-form-item>
              <el-form-item label="是否已打印：" label-width="100px" style="margin-bottom: 0px;margin-top:10px;">
                <el-select v-model="params.tzs_is_print" placeholder="请选择是否打印" style="width:180px">
                  <el-option v-for="item in YesOrNotLists" :key="item.value" :label="item.text" :value="item.value"/>
                </el-select>
              </el-form-item>-->
              <el-form :inline="true" :model="params" size="mini">
        ...
        <el-table
          ...
          <el-table-column fixed="right" align="center" label="操作" width="120">
            <template slot-scope="scope">
              <el-button size="mini" type="success" plain @click="handleChangeUnitRow(scope.$index, scope.row)"> 调整班级</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog v-dialogDrag :visible.sync="dialogFormVisible" width="900px">
      <div slot="title"><i class="el-icon-sort"/> 选择即将调整到的目标班级 </div>
      <el-form ref="changeStudentUnitForm" :rules="rules" :model="form" label-position="top" size="small" >
        <el-form-item label-width="100px" label="目标班级" prop="unit_code">
          <el-cascader v-model="form.unit_code" :options="unitOptions" :props="unitProps" placeholder="请选择目标班级" style="width:800px"/>
        </el-form-item>
        <el-form-item :label-width="formLabelWidth" label="转专业/班级原因：">
...
import { getStudentLists, changeUnitCode,getSearchField } from '@/api/student'
...
export default {
  ...
  mounted() {
    ...
    this.copyParams.pici_id=data.pici_id
    setCurrentUrl(this.$route.path)
    this.fetchPiciData()
    this.fetchExpandedUnitCode()
    //this.fetchUnitLists()
  ...
  methods: {
    ...
    fetchSearchField() {
      getSearchField({ form_name: '分班管理列表' }).then(response => {
   ...
   /* // 加载单位结构数据
    ...
    handleChangeUnit() {
      ...
      if (rows.length === 0) {
        this.$alert('请先选择需要调整班级的学生', '提示', { type: 'warning' })
      } else {
        var ids = []
        rows.forEach(row => {
          ids.push(row.id)
    ...
    handleSubmit() {
      this.$refs.changeStudentUnitForm.validate((valid) => {
        if (valid) {
          ...
          const unit_code = formdata.unit_code[formdata.unit_code.length - 1]
          this.formSumbitLoading = true
          changeUnitCode({ student_ids: student_ids, unit_code: unit_code, remark: formdata.description }).then(response => {
